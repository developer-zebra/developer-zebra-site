  ...
  return function(files, metalsmith, done){
      ...
      if(files[menufile] && files[menufile].menu){
        for (var i = 0; i< files[menufile].menu.length; i++) {
          ...
          for (var x = 0; x < items.length; x++) {
            // console.log(items);
            if(items[x].file){
              var filename = options.folder + items[x].file +'/index.md'
              files[menufile].menu[i].items[x].title =files[filename].title;
              files[menufile].menu[i].items[x].url =files[filename].path.dir.replace(options.folder,'');
    ...
    Object.keys(files).forEach(function(file){
      ...
      if(automenu){
        ...
        if(files[options.folder+'index.md'] && files[options.folder+'index.md'].menu){
          // console.log("****adding menu %s", files[file].path.dir);
    ...
    if(automenu && files[options.folder+'index.md']){
      files[options.folder+'index.md'].menu = menu;
