...
**Scanning Modes -** Used to select between Single (normal), UDI and Basic MultiBarcode modes. [UDI mode](./#udidecoding) acquires multiple simultaneous inputs as specified in the selected Universal Device Identifier UDI parameter(s). [Basic MultiBarcode mode](./#multibarcodedecoding) acquires 2-10 barcodes with each scan as specified in the Basic MultiBarcode params panel. **Default=5**.
