...
**DataWedge keyboard options -** used to display the DataWedge "keyboard" (a DataWedge icon that displaces the default keyboard) to dispatch keystroke data and set the time period for the duration the DataWedge keyboard is displayed. Applies to Android 10 or higher, in which the default keyboard disappears during a scan and reappears after the data is delivered to the app. By enabling this feature, when a scan is performed the default keyboard is displaced with the DataWedge keyboard to dispatch data. After data dispatch is complete and the timeout elapses, the keyboard is switched back to the default keyboard. Zebra recommends to enable this option in the following situations:<br>
