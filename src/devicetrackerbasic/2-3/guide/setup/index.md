...
Device Tracker Basic consists of the following two components:

* **Client** – Device Tracker Basic client app runs on [supported Zebra Android mobile devices](../about/#supporteddevices). It collects device information, sends presence updates to the server, and provides a remote chirping device finding feature. The client app can play the role of either the tracking device or the device being tracked.
* **Server** –  Device Tracker Basic server is part of the Zebra DNA Visibility Console server solution, a common server software for both Device Tracker Basic and [PowerPrecision Console](/ppc/latest/guide/about). It provides a centralized dashboard that is accessible from a supported browser running on a kiosk, tablet, or PC.  The server collects and processes device data for tracking misplaced devices, including tracking status, connection status, and battery status.
...
-->
###Version History
* **Device Tracker Basic 2.3.1 -** Self-signed certificates are now supported to help simplify deployment of product demos and trials.

##System Requirements
This section provides the server and device requirements. Device Tracker Basic supports a maximum of 500 devices per installation.
...
4. Network Access Requirements (see **Server Setup** below):
   * If required, **open incoming and outgoing ports** for communication between server and mobile devices through the server firewall. Sample ports are: 
        * Backend Server: Data port 8080 for Device Tracker Basic Basic client to register to the server and transmit device data 
        * Web Portal: UI port 8443 for accessing Device Tracker Basic web portal  
...
###Device Requirements
Requirements for Device Tracker Basic client:
...
* The server URL, user name, and password is configured in the Device Tracker Basic client to communicate with the server. 
...
3. Create a private key. It prompts to enter the passphrase - _make note of this passphrase_, which is used in Device Tracker Basic. Run the command:  
...
4. Use SSL certificate "ssl_certificate.pfx" and the private key password for Device Tracker Basic server installation and setup in the sections that follow.
...
10. Use self-signed certificate "dtrkdemo.pfx" and the private key password for Device Tracker Basic server installation and setup in the sections that follow.
...
&nbsp; &nbsp; &nbsp;3. **Select app to launch.** As part of ZDVC, the server consists of multiple solution offerings. Select "Device Tracker Basic".<br>
...
This allows Device Tracker Basic to run each time the server restarts regardless of the user logged in.
-->
##Client Install & Setup
Install Device Tracker Basic client on the supported Zebra device to register the device and transmit data to the server. The device must be connected to the same network as the server. Client install and setup can be accomplished either manually or remotely with Zebra's [StageNow](/stagenow/latest/about) or an EMM (Enterprise Mobility Management) system. 
...
1. Download Device Tracker Basic client from [Zebra Support and Downloads](https://www.zebra.com/us/en/support-downloads/software.html). Extract the files:<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;• DTRKClient.apk<br>
...
4. Open Device Tracker Basic client app.
5. Accept the permissions when prompted.
6. Once the app is opened on Android O or higher devices, a Device Tracker Basic notification message is displayed in the device notifications drawer. This notification cannot be dismissed, indicating that Device Tracker Basic is running in the background. 
<img style="height:400px" src="Notifications_dtrk.jpg"/>
_Figure 11. Device Tracker Basic client notification_ <br>
...
> WiFi sleep policy "Keep Wi-Fi on during sleep" must be set to "Always" on Android Marshmallow devices for Device Tracker Basic to work in Doze mode. By default it is set to "Never".

####Manual Configuration
Steps for manual client configuration after installation:
1. Open Device Tracker Basic client.
2. Grant all permissions requested by the app. _If using an SSL certificate,_ proceed to step 4.
3. **If using a self-signed certificate,** open the StageNow application. Scan the barcode from file "EnabledSelfSigned_DTRK.pdf" to enable self-signed certificates in the client app. The .PDF file is included as part of the Device Tracker Basic client installation package.
4. Tap the hamburger menu at the top right, then tap "Settings".  
5. Enter in the following information:
   * **Server URL** - URL for the server with port number and Device Tracker Basic path specified, for example: `hostname.company.com:8080/zdvc/dtrk`, where "hostname.company.com:8080" is replaced with the appropriate hostname, domain and port number. The URL must not contain "https://" nor "http://".
   * **Server Auth Key** - user name designated during server install
   * **Server Auth Password** - password designated during server install
<br>
6. Tap the device back button to save the changes and return to the main screen. Device Tracker Basic client registers with the server and loads "Devices to be found". _If using an SSL certificate,_ manual client configuration is complete.
...
&nbsp;&nbsp;&nbsp;C. To disable self-signed certificates in the app, scan the barcode from "DisableSelfSigned_DTRK.pdf" (part of the Device Tracker Basic client installation package).

####Remote Configuration
After client installation, follow these steps to create StageNow profiles to remotely configure the client:
1. Disable Battery Optimization
2. Start Device Tracker Basic Service
3. Configure Device Tracker Basic settings with CSP
...
1. Install Device Tracker Basic application and start the service.
2. Configure Device Tracker Basic settings.
...
**Steps to create StageNow profile to start Device Tracker Basic service remotely:**
1. Open [StageNow](https://www.zebra.com/us/en/support-downloads/software/utilities/stagenow.html) on a PC. 
2. In the StageNow home screen, click “Create New Profile” from the left menu.  <br>
3. Ensure the proper MX version is selected at the top drop-down selector. This should match the MX version on the device. See [MX documentation](/mx/mx-version-on-device) for instructions how to check the version. <br>Select “XpertMode" from the table. Click Create.<br>
![img](SN_CreateNewProfile.JPG)
...
Due to changes with Android O and above, use **StartActivity** action to start the Device Tracker Basic service. Click Continue.
...
10. For EMM Staging, continue to section "Steps for EMM Staging" below.
11. Open the StageNow client on the device.
12. Scan the barcode generated to start the Device Tracker Basic service in the background.

**Steps for remote client configuration with StageNow and CSP Plug-in:**

1. Download Device Tracker Basic client software DTRKClient.zip from [Zebra Support and Downloads](https://www.zebra.com/us/en/support-downloads/software/productivity-apps/power-precision-console.html). The .zip file includes the following: 
...
1. Pre-requisite steps:
   * Follow procedure for "Device Tracker Basic remote configuration with StageNow and CSP Plug-in" up to step 6
