...
* **Press CTRL-Z to Undo the most recent operations(s)** in reverse order. 
...
**The following conditions must be met before clicking Deploy**: 

* **Development host**:
 * Must have [Android Debug Bridge (adb)](https://developer.android.com/studio/command-line/adb)** installed and operable
 * <u>No more than one target device connected</u>
 * Current layout and project must be saved
* **Target device**: 
 * Enterprise Keyboard (3.4 or higher) installed and enabled as default input source

##### Layout files can be deployed in one of three ways: 

1. **Push a layout file via [Android Debug Bridge (adb)](https://developer.android.com/studio/command-line/adb)** from the development host to device(s)
2. **Press the "Deploy" button** to push the current layout file to a host-connected device via USB
3. **Use an Enterprise Mobile Management (EMM) system** for mass deployment
