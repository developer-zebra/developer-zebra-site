...
10. Now we need to use the `onOpened` method to get a reference to the [EMDKManager](../../api/barcode/EMDKManager). The [EMDKListener](../../api/barcode/EMDKManager-EMDKListener) interface will trigger this event when the EMDK is ready to be used. The [EMDKListener](../../api/barcode/EMDKManager-EMDKListener) interface must be implemented in order to get a reference to the EMDKManager APIs. This event will pass the EMDKManager instance and we assign it to the global variable `emdkManager` that we created in the earlier steps. We have used that instance to get an instance of [Barcode Manager](../../api/barcode/BarcodeManager) to enable scanning.
