...
* The profile name will be used to inform the ProfileManager methods to apply specific profile on the device. The modified date and target MX version are used for profile maintenance such as editing, renaming and modifying profiles in the profile manager wizard. This will not set on the device.

* Each profile will have one or more characteristic feature nodes with parm settings specific to taht feature. An example is show below. 
...
##Applying Profiles
The  API exposes a method ( processProfile ) that is used
...
The [ProfileManager](/emdk-for-android/5-0/api/core/ProfileManager/) API exposes a method (processProfile and processProfileAsync) that is used to apply the XML profile located in your Application’s Assets folder or optional profile data passed as extraData to these methods. 
