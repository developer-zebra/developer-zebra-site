...
* **[Decoder Params](/emdk-for-android/6-10/api/barcode/ScannerConfig-DecoderParams):**
...
* **[Reader Params](/emdk-for-android/6-10/api/barcode/ScannerConfig-ReaderParams):**
...
* **[Scan Params](/emdk-for-android/6-10/api/barcode/ScannerConfig-ScanParams):**
...
Create a new project with Android Studio, click [here](/emdk-for-android/6-10/tutorial/tutCreateProjectAndroidStudio).
...
1. Now we will start to add some code. 
    ...
    We will now create some global variables to hold the instance objects of EMDKManager, [BarcodeManager](/emdk-for-android/6-10/api/barcode/BarcodeManager) and [Scanner](/emdk-for-android/6-10/api/barcode/Scanner). These variables would be used throughout the code.
...
8. Now, we will add `setProfile` method. This method will create an instance of [ScannerConfig](/emdk-for-android/6-10/api/barcode/ScannerConfig), which will allow us to modify scanner properties (Ex. [Decoder Params](/emdk-for-android/6-10/api/barcode/ScannerConfig-DecoderParams), [Reader Params](/emdk-for-android/6-10/api/barcode/ScannerConfig-ReaderParams), [Scan Params](/emdk-for-android/6-10/api/barcode/ScannerConfig-ScanParams) etc.) and set the changes back to the scanner instance. Here, we will read the checkboxes and spinner values and set them accordingly to the scanner instance through [ScannerConfig](/emdk-for-android/6-10/api/barcode/ScannerConfig). `config` is name of [ScannerConfig](/emdk-for-android/6-10/api/barcode/ScannerConfig) instance.
    ...
    `config.decoderParams` allows us to set all the decoder params such as `code11`,`code39` etc. `readerParams.readerSpecific.cameraSpecific.illuminationMode` enables user to set [Illumination Mode](/emdk-for-android/6-10/api/barcode/ScannerConfig/#scannerconfigilluminationmode), which is available only for CameraSpecific devices. `config.scanParams.decodehapticfeedback` lets you set [Vibration Mode](/emdk-for-android/6-10/api/barcode/ScannerConfig-ScanParams#decodehapticfeedback) of the scanning device. `config.scanParams.decodeAudioFeedbackUri` will accept a URI for the [Audio Tone](/emdk-for-android/6-10/api/barcode/ScannerConfig-ScanParams/#decodeaudiofeedbackuri) that user needs to set while scanning a barcode.
...
10. Now we need to use the `onOpened` method to get a reference to the [EMDKManager](/emdk-for-android/6-10/api/core/EMDKManager). The [EMDKListener](/emdk-for-android/6-10/api/core/EMDKManager-EMDKListener) interface will trigger this event when the EMDK is ready to be used. The [EMDKListener](/emdk-for-android/6-10/api/core/EMDKManager-EMDKListener) interface must be implemented in order to get a reference to the EMDKManager APIs. This event will pass the EMDKManager instance and we assign it to the global variable `emdkManager` that we created in the earlier steps. We have used that instance to get an instance of [Barcode Manager](/emdk-for-android/6-10/api/barcode/BarcodeManager) to enable scanning.
...
That's it!!! We are done with all the coding part that will let us perform some advanced barcode scanning operations on our Symbol Android device using [Barcode/Scanning APIs](/emdk-for-android/6-10/api) introduced in EMDK V 3.0. Now let us run the application.
...
4. Now once again scan a barcode and this time it will open your device's camera to scan the barcode.
    ...
    This is how [Barcode/Scanning APIs](/emdk-for-android/6-10/api) introduced in EMDK V 3.0 can be used to perform some of the Advanced Scanning operations using soft scan key through application on your Symbol devices without using Profile Wizard. 
