...
Enterprise Browser is a powerful, industrial browser that enables developers to build feature-rich web applications that integrate seamlessly with the features in Zebra mobile computers and peripherals. The  base installation includes everything needed to quickly build device apps for barcode scanning, signature capture, printing and most other enterprise scenarios. Enterprise Browser applications are built with standard web technologies such as HTML5, CSS3 and JavaScript, and run and integrate with virtually any Zebra device running Android and Microsoft Windows Mobile/CE. 
...
[See all supported devices](/enterprise-browser/1-4/guide/about?Mobile)
...
###New Features
* **[Keycode Mapping](/enterprise-browser/1-4/guide/keycapture)** allows a custom set of Android keycode values to be assigned from a file, replacing some or all of the system's default keycode values. **Applies to Android only**.


###New Tags

* **[&lt;KeepAlive&gt;](/enterprise-browser/1-4/guide/configreference?KeepAlive)** provides the option of maintaining a connection from client to server between HTTP requests. **For Windows Mobile/CE with Webkit engine only**. 

* **[&lt;LogTrace&gt;](/enterprise-browser/1-4/guide/configreference?LogTrace)** provides logging of the Trace messages generated by Enterprise Browser. **For Android and Windows Mobile/CE**.

* **[&lt;usedwforscanning&gt;](/enterprise-browser/1-4/guide/configreference?usedwforscanning)** controls whether to 'use DataWedge (DW) for scanning' or to go through Enterprise Browser APIs. Refer to the [DataWedge Usage Guide](/enterprise-browser/1-4/guide/datawedge) for details. **For Android only**.

###New Guides
* **[DataWedge Usage Guide](/enterprise-browser/1-4/guide/datawedge)** details the steps required to use DataWedge for barcode scanning instead of the Enterprise Browser APIs and for switching between the two. 

* **[Keycode Mapping Guide](/enterprise-browser/1-4/guide/keycapture)** explains how to remap Android keys to ensure that  keycode values are delivered to apps as expected. 
