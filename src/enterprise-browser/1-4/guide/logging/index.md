...
**&ogUser -** Controls logging of User, Info, Warning and Error messages from the user application (overrides LogWarning, LogError and/or LogInfo settings). Data can be logged using the Log API.
...
The &lt;Logger&gt; section of the `Config.xml` file is shown below with its default settings. To enable a logging of a particular event, change the value in its tag to "1" as shown below for Error, Warning and Info events:
...
### Logging Options
The logging function offers the following additional options: 

**ogProtocol -** Sets the protocol over which the logging data will be sent (accepts any valid URL or fully qualified path and file name).  

**LogPort -** The port over which the logging data will be sent when HTTP is set in LogProtocol; otherwise ignored (accepts any valid HTTP port). 
...
**LogMemPeriod -** Specifies the time interval after which memory logs will be generated. Supports Android and WM/CE; does not apply to the Enterprise Tablet (accepts an integer as a length of time, in ms). 

**LogMaxSize -** Specifies the maximum allowable size of the log file, after which no more logs will be saved (accepts an integer as the file size, in KB). 
