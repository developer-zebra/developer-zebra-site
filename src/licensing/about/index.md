...
### DataWedge
DataWedge allows any application on a device to acquire data from the device scanner(s) and other input sources ***without coding***, and to manipulate the data as needed based on simple options or complex rules. DataWedge is available out-of-the-box with every Zebra Android device. [Learn more](/datawedge).

#### Multi-barcode scanning
Implemented as [NextGen SimulScan](http://techdocs.zebra.com/datawedge/latest/guide/input/barcode#scanningmodes) in DataWedge, **multi-barcode options** currently available for licensing include [Number of barcodes per scan](https://techdocs.zebra.com//datawedge/latest/guide/input/barcode#basicmultibarcodeparams); [Instant reporting](https://techdocs.zebra.com//datawedge/latest/guide/input/barcode#basicmultibarcodeparams), which instantaneously reports unique barcodes within a scanning session; and [Report decoded barcodes](http://techdocs.zebra.com/datawedge/latest/guide/input/barcode#basicmultibarcodeparams) for reporting all barcodes decoded in a single scan session. 

-----

### Device Diagnostics
Device Diagnostic Tool instantly tests and diagnoses the hardware operability of a Zebra mobile device to determine system health and functionality. Where appropriate, Zebra Help Desk uses this tool for troubleshooting device issues, relying on the results to provide optimum steps for resolution. DDT is useful for quickly troubleshooting device issues, increasing worker productivity by reducing device downtime and unnecessary returns to the Zebra Repair Center. [Learn more](/ddt).
...
* [Support page](https://www.zebra.com/us/en/support-downloads/software/productivity-apps/worryfree-wifi.html) | Supported devices, administrator guides
* [MX page](/mx/worryfreewifimgr) | Remote management, feature control through StageNow, EMM
