...
The Long Term Storage (LTS) module gathers log files generated by all other active modules and stores them together for analysis of device health and behavior over a period of time.
...
Date is stored in YYMMDD and time is stored in HH00 using military time.
...
output
Qxdm is unique in the fact that it will not contain data up to a limit before removing the oldest. means it will infinately grow 


If Qxdm is started and not stopped it can take up an extreme amount of memory.
...
for orgs using qc util to create their own config, put file here. 
...
Captures the last kmsg from the device, writing it to persistent memory immediately prior to a system crash. Ramoops outputs text files generated by the system at boot or after a kernel panic. These files will have raw kmsg data from the boot immediately prior to the crash.
...
more precise data less frequently such as threasds dumpsys details.  

Why "snapshot"? 

What's a system "fingerprint" ?  which includes kernel version, BSP build number and otehr vital stats. 
