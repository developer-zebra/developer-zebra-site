...
**StageNow v4.0 supports all Zebra devices with the following operating systems and extensions**: 
...
Limits device staging to approved and certified barcode sources 

Note: Trusted Staging is Optional, Trusted profiles can be created with Mx 9.2 onwards. 

MX 9.2 support: integration with the latest MX version adds the following enhancements: 

Power Manager new features: 

Controls individual hardware wake up methods (buttons) 

Display Manager new features: 

Controls device font and display size 

Access Manager new features: 

Offers enhanced device visibility and control while remote troubleshooting 
